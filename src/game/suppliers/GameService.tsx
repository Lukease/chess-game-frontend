...
import GameNavigation from '../../UI/start-game/GameNavigation'
import AddPiecePanel from '../../UI/new-figure/AddPiecePanel'

export class GameService {
    static gameNavigation = GameNavigation
    addPiecePanels = AddPiecePanel
     board = Board
     arena = Arena
    static promotePawnPanel = PromotePawnPanel
    static historyOfMoves = HistoryOfMoves
    ...
    fieldClick(field: Field) {
        ...
        if (this.fieldHasPieceAndGameStarted(field) && this.isPlayerPiece(field.piece!)) {
            ...
            this.arena.setInterfaceColor()
            this.board.setTrashToggle()
        }
    ...
    getSpecialMovesForPiece(field: Field): Array<Move> {
        return field.piece!.getSpecialMoves().map((move: MoveType) => this.specialMovePossible(move, field))
    ...
    getRookToCastle(field: Field, smallCastle: boolean): boolean {
        ...
        return this.getFieldByXY(rookX, field.coordinate.y)!.piece?.hasMoved as boolean
    }
    ...
    getFieldForCastle(field: Field, smallCastle: boolean) {
        if (!this.getRookToCastle(field, smallCastle)
            && this.checkCastleEmptyFields(field, smallCastle)
    ...
    getCorrectMoves(currentField: Field): Array<Move> {
        return currentField.piece!.getAllPossibleDirectionsWithColor()
            .map((direction: Vector2d) => this.getAllPossibleMovesFromDirection(currentField, direction))
            .flat(1)
            .map((move: Field) => new Move(undefined, currentField, undefined, move, undefined, '',
    ...
    canPreventCheck() {
        ...
        return arrayOfPieceWithDirections.map(pieceWithDirections => {
            return pieceWithDirections!.directions.map((direction: Vector2d) => {
    ...
    getAllPossibleMoves(fieldsArray: Array<Field>) {
        return (
            fieldsArray!.map(field => {
                const correctFields = field.piece!.getAllPossibleDirectionsWithColor()
                    .map((direction: Vector2d) => this.getAllPossibleMovesFromDirection(field, direction))
